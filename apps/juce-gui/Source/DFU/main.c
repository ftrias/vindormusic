...
static int parse_number(char *str, char *nmb)
{
	...
	if ((errno == ERANGE && (val == LONG_MAX || val == LONG_MIN))
		...
		errxx(EX_SOFTWARE, "Something went wrong with the argument of --%s\n", str);
	}

	if (endptr == str) {
		errxx(EX_SOFTWARE, "No digits were found from the argument of --%s\n", str);
	}
	...
	if (!(dfu_root->flags & DFU_IFF_DFU)) {
		...
		* DFU Interface descriptor according to the DFU Spec. */
		...
		if (libusb_claim_interface(dfu_root->dev_handle, dfu_root->interface) < 0) {
			errxx(EX_IOERR, "Cannot claim interface %d",
				dfu_root->interface);
		}
...
dfustate:
	...
	if (libusb_set_configuration(dfu_root->dev_handle, dfu_root->configuration) < 0) {
		errxx(EX_IOERR, "Cannot set configuration");
	}
#endif
