...
SF2Reader::SF2Reader(SF2Sound* soundIn, const File& fileIn, InputStream* stream)
    : sound(soundIn), myFile(true)
{
    if (stream) {
        file = stream;
        myFile = false;
        ...
        file_ref = fileIn.createInputStream();
        file = file_ref.get();
        myFile = false;  //  ????
    }
}
	...
	double* progressVar, Thread* thread)
{
	...
	while (samplesLeft > 0) {
		...
		file->read(buffer, (size_t)samplesToRead * sizeof(short));
